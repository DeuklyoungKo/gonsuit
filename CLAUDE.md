...
```
./                             ← 저장소 루트 (문서 + 웹 프로젝트)
├── 1_PRD.md / 2_PDP.md / 4_LRP.md / CLAUDE.md
├── supabase_schema.sql
...
> 📘 **규칙 원문: [`0_DOC_CONVENTION.md`](0_DOC_CONVENTION.md)**
> SSOT 원칙 · 갱신 트리거 · 스냅샷 원칙 · 경로 표기 규칙 · Notion↔GitHub 경계 · 안티패턴은
> **모두 그 문서가 소유한다. 여기서 반복하지 않는다.** (이 프로젝트가 규약의 레퍼런스 구현이다.)

**이 프로젝트의 문서 구성** — 위 규약을 적용한 결과다.

| 문서 | 이 프로젝트에서의 상태 |
|---|---|
| **`CLAUDE.md`** (이 문서) | **현재값 SSOT + 허브.** 기술 스택 · 디렉토리/페이지 구조 · 개발 상품 현황 · GA4/SEO 규칙 · 코딩 컨벤션 · 커밋 규칙 |
| `1_PRD.md` | 제품 요구사항 (왜/무엇) |
| `2_PDP.md` | ✅ **1차 런칭 완료 — 동결.** 이후 작업은 이 문서가 관리 |
| `3_DesignGuide.md` | **없음** — 웹 프로젝트라 별도 디자인 가이드를 두지 않고 번호를 비워 둠 |
| `4_LRP.md` | 장기 로드맵 (Phase 1~3) |
| `dev.md` | **없음** — 필요해지면 사람(Gon)이 직접 만든다. AI는 만들지 않는다 |

---

## 참조 문서

- `0_DOC_CONVENTION.md` — **문서 관리 규약** (다른 프로젝트 적용 시 이 문서를 따른다)
- `1_PRD.md` — 제품 요구사항 정의서 (왜/무엇)
- `2_PDP.md` — 개발 계획서 (1차 런칭 완료, 아카이브)
- `4_LRP.md` — 장기 로드맵 (전략/비전)
